...
up<std::vector<coord>> valid_neighbors(const size_t r, const size_t c,
                                       const ptrdiff_t max_r,
                                       const ptrdiff_t max_c,
...
void etch(mat_t<int_fast8_t> &mat, const bool wrap = true) {
  ...
  up<mat_t<up<std::vector<coord>>>> neighbors(
      new mat_t<up<std::vector<coord>>>());
...
double anneal_step(mat_t<int_fast8_t> &mat, const double T,
                   const mat_t<up<std::vector<coord>>> &moves, const path &p,
...
int main(int argc, char *argv[]) {
  ...
  for (uint32_t step = step_offset; step <= MAX_STEPS + step_offset; step++) {
    ...
    if (step % 10000 == 0) {
      ...
      if (ofs.fail()) {
        std::cout << "Failed to open a file at step " << std::to_string(step)
                  << '\n';
      } else {
